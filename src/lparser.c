...
#ifdef PALMOS
  long breaklist;  /* list of jumps out of this loop */
#else
...
static void close_func (LexState *ls) {
  ...
  f->sizecode = fs->pc;
#ifdef PALMOS
  luaM_reallocvector(L, f->lineinfo, f->sizelineinfo, fs->pc, long);
#else
...
static void recfield (LexState *ls, struct ConsControl *cc) {
  ...
  if (ls->t.token == TK_NAME) {
    luaX_checklimit(ls, cc->nh, 32767, "items in a constructor");
    cc->nh++;
    checkname(ls, &key);
  }
  else  /* ls->t.token == '[' */
