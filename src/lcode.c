...
void luaK_nil (FuncState *fs, Argument from, Argument n) {
  ...
  if (fs->pc > fs->lasttarget &&  /* no jumps to current position? */
    ...
    Argument pfrom = GETARG_A(*previous);
    Argument pto = GETARG_B(*previous);
...
Argument luaK_jump (FuncState *fs) {
  Argument jpc = fs->jpc;  /* save list of jumps to here */
  Argument j;
  fs->jpc = NO_JUMP;
...
static Argument luaK_condjump (FuncState *fs, OpCode op, Argument A, Argument B, Argument C) {
  luaK_codeABC(fs, op, A, B, C);
  return luaK_jump(fs);
}


static void luaK_fixjump (FuncState *fs, Argument pc, Argument dest) {
  Instruction *jmp = &fs->f->code[pc];
  Argument offset = dest-(pc+1);
  Argument abs_offset = offset < 0 ? -offset : offset;
  lua_assert(dest != NO_JUMP);
  if (abs_offset > MAXARG_sBx)
...
static Argument luaK_getjump (FuncState *fs, Argument pc) {
  Argument offset = GETARG_sBx(fs->f->code[pc]);
...
static Instruction *getjumpcontrol (FuncState *fs, Argument pc) {
...
*/
static Argument need_value (FuncState *fs, Argument list, Argument cond) {
...
static void patchtestreg (Instruction *i, Argument reg) {
...
static void luaK_patchlistaux (FuncState *fs, int list, int vtarget, int reg,
                               int dtarget) {
  while (list != NO_JUMP) {
    Argument next = luaK_getjump(fs, list);
    Instruction *i = getjumpcontrol(fs, list);
...
void luaK_checkstack (FuncState *fs, Argument n) {
  Argument newstack = fs->freereg + n;
...
static void freereg (FuncState *fs, Argument reg) {
...
static Argument addk (FuncState *fs, TObject *k, TObject *v) {
  const TObject *idx = luaH_get(fs->h, k);
  if (ttisnumber(idx)) {
    lua_assert(luaO_rawequalObj(&fs->f->k[cast(Argument, nvalue(idx))], v));
    return cast(Argument, nvalue(idx));
  }
...
static Argument nil_constant (FuncState *fs) {
  TObject k, v;
  setnilvalue(&v);
...
void luaK_setcallreturns (FuncState *fs, expdesc *e, Argument nresults) {
...
static Argument code_label (FuncState *fs, Argument A, Argument b, Argument jump) {
  luaK_getlabel(fs);  /* those instructions may be jump targets */
  return luaK_codeABC(fs, OP_LOADBOOL, A, b, jump);
}


static void discharge2reg (FuncState *fs, expdesc *e, Argument reg) {
...
static void luaK_exp2reg (FuncState *fs, expdesc *e, Argument reg) {
  discharge2reg(fs, e, reg);
  if (e->k == VJMP)
    luaK_concat(fs, &e->t, e->info);  /* put this jump in `t' list */
  if (hasjumps(e)) {
    Argument final;  /* position after whole expression */
    Argument p_f = NO_JUMP;  /* position of an eventual LOAD false */
    Argument p_t = NO_JUMP;  /* position of an eventual LOAD true */
    if (need_value(fs, e->t, 1) || need_value(fs, e->f, 0)) {
      Argument fj = NO_JUMP;  /* first jump (over LOAD ops.) */
      if (e->k != VJMP)
        fj = luaK_jump(fs);
...
void luaK_storevar (FuncState *fs, expdesc *var, expdesc *exp) {
  switch (var->k) {
    ...
    case VUPVAL: {
      Argument e = luaK_exp2anyreg(fs, exp);
...
static Argument jumponcond (FuncState *fs, expdesc *e, Argument cond) {
  if (e->k == VRELOCABLE) {
...
static void codenot (FuncState *fs, expdesc *e) {
  ...
  { Argument temp = e->f; e->f = e->t; e->t = temp; }
  removevalues(fs, e->f);
  removevalues(fs, e->t);
}
...
static void codebinop (FuncState *fs, expdesc *res, BinOpr op,
  ...
  if (op <= OPR_POW) {  /* arithmetic operator? */
    ...
    Argument cond = 1;
    if (op >= OPR_GT) {  /* `>' or `>='? */
      Argument temp;  /* exchange args and replace by `<' or `<=' */
...
void luaK_posfix (FuncState *fs, BinOpr op, expdesc *e1, expdesc *e2) {
  switch (op) {
    ...
    default: {
      Argument o1 = luaK_exp2RK(fs, e1);
      Argument o2 = luaK_exp2RK(fs, e2);
      freeexp(fs, e2);
      freeexp(fs, e1);
...
Argument luaK_code (FuncState *fs, Instruction i, Argument line) {
  ...
  luaM_growvector(fs->L, f->lineinfo, fs->pc, f->sizelineinfo, Argument,
                  MAX_INT, "code size overflow");
...
Argument luaK_codeABx (FuncState *fs, OpCode o, Argument a, unsigned Argument bc) {
